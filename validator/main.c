...
//	read_gdb_output_pc(drive_gdb_reply_buffer, &loader_list[6]); //on demande � gliss de charger la pile au m�me endroit que GDB
	
//	real_state=iss_init(mem_list,loader_list,system_list,NULL,NULL);

	//gliss est stopp� au tout d�but (_start), gdb est quelques instructions plus loin, donc il faut avancer gliss
...
/*		iss_fetch(NIA(real_state),buff_instr);
	...
	//maintenant il faut �craser les registres de GDB avec ceux de GLISS
...
int init_gdb(char * drive_gdb_reply_buffer, char * target)
	...
	/*Il faut balancer tout le blabla de départ et attendre l'invite*/
	...
	if ( strcmp(target, "local_gdb\n") ) //si on ne travaille pas en local
		...
		/*Procédure décrite dans la doc : on met un breakpoint sur main, on vérifie que gdb est ok*/
		...
		wait_for_gdb_output(drive_gdb_reply_buffer, display_replies); //on doit attendre le message d'arrêt au bkpt
	...
	create_gdb_vars(drive_gdb_reply_buffer); //g�n�r� par le script python		
...
int main(int argc, char ** argv)
	...
	/* On crée les deux tubes pour communiquer avec gdb */
	if ( pipe(to_gdb_pipe) )
		{
...
void drive_gdb()
	...
	/* On redirige l'entrée de GDB sur le pipe */
