...
int init_gdb(char * drive_gdb_reply_buffer, char * target)
	...
	/*Il faut balancer tout le blabla de d̩part et attendre l'invite*/
	...
	if ( strcmp(target, "local_gdb\n") ) //si on ne travaille pas en local
		...
		/*Proc̩dure d̩crite dans la doc : on met un breakpoint sur main, on v̩rifie que gdb est ok*/
		...
		wait_for_gdb_output(drive_gdb_reply_buffer, display_replies); //on doit attendre le message d'arr̻t au bkpt
...
int main(int argc, char ** argv)
	...
	/* On cr̩e les deux tubes pour communiquer avec gdb */
	if ( pipe(to_gdb_pipe) )
		{
...
void drive_gdb()
	...
	/* On redirige l'entr̩e de GDB sur le pipe */
