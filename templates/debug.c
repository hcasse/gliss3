...
$(foreach registers)$(if !aliased)
	{
		...
		$(format),
		$(size),
		$(if is_pc)TYPE_ADDR$(else)$(if is_float)RTYPE_FLOAT$(else)TYPE_INT$(end)$(end),
		$(type_size)
	},
$(end)$(end)	{
		-1
...
$(foreach registers)$(if !aliased)
	case $(id):
		$(PROC)_$(NAME)$(if array)[idx]$(end) = r.$(if !is_float)$(if !is_64)iv$(else)lv$(end)$(else)$(if !is_64)fv$(else)dv$(end)$(end);
		break;
$(end)$(end)
	default:
		assert(0);
