(*
 * $Id: gep.ml,v 1.11 2009/01/13 14:04:31 casse Exp $
...
(** Build a file (assign the info.Toc.out to use)
	and insert "generated by" banner at head.
	@param f	Function to make content.
	@param info	Generation information.
	@param path	Path of the file. *)
let make_c_file f info path =
...
let make_makefile info =
	Templater.generate (make_env info) "Makefile" "src/Makefile"
...
let make_id_h info =
	...
	Templater.generate (make_env info) "id.h" ("include/" ^ proc ^ "/id.h")
...
let make_api_h info =
	...
	Templater.generate dict "api.h" ("include/" ^ proc ^ "/api.h")
...
let make_macros_h info =
	let dict = make_env info in
	Templater.generate dict "macros.h" ("include/" ^ info.Toc.proc ^ "/macros.h")
...
(** Build the file src/state.h
	@param info	Generation information. *)
let make_state_header info =
	let out = info.Toc.out in
	let proc = info.Toc.proc in
	let uproc = String.uppercase proc in
	Printf.fprintf out "#ifndef %s_STATE_H\n" uproc;
	Printf.fprintf out "#define %s_STATE_H\n" uproc;
	Printf.fprintf out "\n#include \"../include/%s/api.h\"\n" proc;
	List.iter
		(fun (name, _) -> Printf.fprintf out "#include \"%s.h\"\n" name)
		!modules;
	Printf.fprintf out "\ntypedef struct %s_full_state_t {\n" proc;
	Printf.fprintf out "	%s_state_t state;\n" proc;
	List.iter
		(fun (name, _) -> Printf.fprintf out "	%s_%s_STATE\n" uproc (String.uppercase name))
		!modules;
	Printf.fprintf out "} %s_full_state_t;\n" proc;
	Printf.fprintf out "\n#endif /* %s_STATE_H */\n" uproc
...
let _ =
	try	
		begin
			...
			make_makefile info;
			make_c_file make_state_header info (info.Toc.spath ^ "/state.h");
